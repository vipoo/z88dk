...
void assemble_file( char* filename )
{
    ...
    if (strcmp(filename, obj_filename) == 0 &&          /* input is object file */
        ...
        if (file_exists(filename)) {
            src_filename =
                filename;                        /* use whatever extension was given */
        }
        else {
            const char* asm_filename = get_asm_filename(filename);

            if (file_exists(
                    asm_filename))                  /* file with .asm extension exists */
...
static void query_assemble(const char* src_filename )
{
    ...
    if ( opts.date_stamp &&                                 /* -d option */
         ...
         objmodule_loaded(
             obj_filename)                  /* object file valid and size loaded */
       ) {
...
const char* GetLibfile( const char* filename )
{
    ...
    newlib->libfilename = m_strdup(
                              found_libfilename );        /* freed when newlib is freed */

    if (!check_library_file(
            found_libfilename))                 /* not a library or wrong version */
        return NULL;
...
NewLibrary( void )
{
    ...
    if ( libraryhdr->firstlib == NULL ) {
        ...
        libraryhdr->currlib->nextlib =
            newl;    /* current/last library points now at new current */
