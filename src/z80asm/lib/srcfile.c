...
typedef struct FileStackElem {
    ...
    const char*
    line_filename;          /* source file name of LINE statement, held in strpool */
...
/* call callback */
static void call_new_line_cb(const char* filename, int line_nr,
                             const char* text )
{
    if ( new_line_cb != NULL )
...
*----------------------------------------------------------------------------*/
static incl_recursion_err_cb_t incl_recursion_err_cb =
    NULL;        /* default handler */

/* set call-back for input/output error; return old call-back */
incl_recursion_err_cb_t set_incl_recursion_err_cb( incl_recursion_err_cb_t
        func )
{
...
const char* SrcFile_filename( SrcFile* self )
{
    return self->line_filename;
}
int         SrcFile_line_nr(  SrcFile* self )
{
    return self->line_nr;
}
