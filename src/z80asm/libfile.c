...
void make_library(const char* lib_filename, UT_array* src_files) {
    ...
    xfwrite_str(Z80libhdr, lib_file);

    /* write each object file */
    char** pfile = NULL;
    while ((pfile = (char**)utarray_next(src_files, pfile)) != NULL) {
        fptr = ftell( lib_file );
        ...
        if (utarray_next(src_files, pfile) == NULL)
            xfwrite_int32(-1, lib_file);
        else
            xfwrite_int32(fptr + 4 + 4 + obj_size,  lib_file);

        /* write module size */
        xfwrite_int32(obj_size, lib_file);
