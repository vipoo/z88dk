...
bool
source_list_ended;     /* end of source listing, from now on no source lines */

/* current line being output */
bool
line_started;          /* true if a line was started but not ended */
long
start_line_pos;        /* ftell() position at start of next list line */
...
extern void ListFile_patch_data( ListFile* self, long patch_pos, long value,
                                 int num_bytes );
