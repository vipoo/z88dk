...
int
align;             // if align>1, section is aligned at align-boundaries
...
// of each module
int
asmpc_phase;       // asmpc within a PHASE/DEPHASE block, -1 otherwise
int          opcode_size;       // number of bytes added after last
// set_PC() or next_PC()
ByteArray*   bytes;             // binary code of section, used to compute
// current size
intArray*
reloc;             // list of addresses in module containg relocable addreses
...
extern void  patch_word_be(int addr,
                           int word);         /* 2-byte word big endian*/
...
*----------------------------------------------------------------------------*/
extern void fwrite_codearea(const char* filename, FILE** pbinfile,
                            FILE** prelocfile );
