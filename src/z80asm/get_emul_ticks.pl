	...
	for my $carry (0..1) {
        ...
        eval {
            ...
            for (path("test.lis")->lines) {
                if (/^\d+\s+([0-9A-F]{4})\s+;START/) { $start = $1; }
                if (/^\d+\s+([0-9A-F]{4})\s+00\s+nop;END/) { $end = $1; }
            }
            defined($start) or die "start address not found\n";
            defined($end) or die "end address not found\n";

            run("z88dk-ticks -m$cpu -start $start -end $end test.bin >test.out");	
