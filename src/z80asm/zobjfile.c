...
static long write_expr( FILE* fp ) {
    ...
    for ( iter = ExprList_first( CURRENTMODULE->exprs ); iter != NULL; iter = ExprList_next( iter ) ) {
        ...
        xfwrite_uint8(range, fp);				/* range of expression */
        ...
        if ( expr->location.filename != NULL &&
            ...
            xfwrite_count16str_str(expr->location.filename, fp);
            Str_set( last_sourcefile, expr->location.filename );
        }
        else
            xfwrite_count16str_str("", fp);

        xfwrite_int32(expr->location.line_num, fp);		/* source line number */

        xfwrite_count8str_str(expr->section->name, fp);	/* section name */

        xfwrite_uint16(expr->asmpc, fp);					/* ASMPC */
        xfwrite_uint16(expr->code_pos, fp);				/* patchptr */
        xfwrite_count8str_str(target_name, fp);			/* target symbol for expression */
        xfwrite_count16str_str(Str_data(expr->text), fp);	/* expression */
    }

    xfwrite_uint8(0, fp);								/* terminator */
...
static int write_symbols_symtab( FILE* fp, SymbolHash* symtab ) {
    ...
    for ( iter = SymbolHash_first( symtab ); iter; iter = SymbolHash_next( iter ) ) {
        ...
        if (scope != 0 && sym->is_touched && sym->type != TYPE_UNKNOWN) {
            ...
            xfwrite_uint8(scope, fp);
            xfwrite_uint8(type, fp);

            xfwrite_count8str_str(sym->section->name, fp);
            xfwrite_int32(sym->value, fp);
            xfwrite_count8str_str(sym->name, fp);

            // write symbol definition location
            xfwrite_count8str_str(sym->location.filename ? sym->location.filename : "", fp);
            xfwrite_int32(sym->location.line_num, fp);
...
static long write_modname( FILE* fp ) {
    ...
    xfwrite_count8str_str(CURRENTMODULE->modname, fp);		/* write module name */
    return modname_ptr;
}
...
static long write_code( FILE* fp ) {
    ...
    if (opts.verbose)
        printf("Module '%s' size: %d bytes\n", CURRENTMODULE->modname, code_size);
...
void write_obj_file(const char* source_filename ) {
    ...
    xfwrite_int32(modname_ptr, fp);
    xfwrite_int32(expr_ptr, fp);
    xfwrite_int32(symbols_ptr, fp);
    xfwrite_int32(externsym_ptr, fp);
    xfwrite_int32(code_ptr, fp);
...
OFile* OFile_read_header( FILE* file, size_t start_ptr ) {
    UT_string* modname = utstring_alloc();
    OFile* self;
    ...
    self->modname_ptr	= xfread_int32(file);
    self->expr_ptr		= xfread_int32(file);
    self->symbols_ptr	= xfread_int32(file);
    self->externsym_ptr	= xfread_int32(file);
    self->code_ptr		= xfread_int32(file);

    /* read module name */
    fseek( file, start_ptr + self->modname_ptr, SEEK_SET );
    xfread_count8str_utstring(modname, file);
    self->modname		= str_pool_add(utstring_body(modname));

    utstring_free(modname);
    ...
    if ( ofile != NULL ) {
        ...
        if ( ofile->code_ptr >= 0 ) {
            ...
            while (true) {	/* read sections until end marker */
                code_size = xfread_int32(ofile->file);
                if ( code_size < 0 )
                    break;

                /* reserve space in section */
                xfread_count8str_utstring(section_name, ofile->file);
                section = new_section(utstring_body(section_name));
                read_origin(ofile->file, section);
                section->align = xfread_int32(ofile->file);
