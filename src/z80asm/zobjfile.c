...
{
    STR_DEFINE(last_sourcefile,
               STR_SIZE);      /* keep last source file referred to in object */
    ExprListElem* iter;
    Expr* expr;
    char range;
    ...
    for ( iter = ExprList_first( CURRENTMODULE->exprs ); iter != NULL;
        ...
        xfwrite_bcount_cstr(target_name,
                            fp);           /* target symbol for expression */
...
static int write_symbols_symtab( FILE* fp, SymbolHash* symtab )
{
    ...
    for ( iter = SymbolHash_first( symtab ); iter;
        ...
        /* scope */
        scope =
            (sym->scope == SCOPE_PUBLIC || (sym->is_defined
                                            && sym->scope == SCOPE_GLOBAL)) ? 'G' :
...
static long write_externsym( FILE* fp )
{
    ...
    for ( iter = SymbolHash_first( global_symtab ); iter;
          iter = SymbolHash_next( iter ) ) {
        sym = ( Symbol* )iter->value;

        if (sym->is_touched &&
            (sym->scope == SCOPE_EXTERN || (!sym->is_defined
...
void OFile_copy( OFile* self, OFile* other )
{
    xassert(0);
}
