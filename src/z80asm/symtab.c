...
void SymbolHash_cat( SymbolHash** ptarget, SymbolHash* source )
{
    ...
    for ( iter = SymbolHash_first( source ); iter;
...
*----------------------------------------------------------------------------*/
Symbol* _define_sym(const char* name, long value, sym_type_t type,
                    sym_scope_t scope,
...
Symbol* get_used_symbol(const char* name )
{
    ...
    sym = find_symbol( name,
                       CURRENTMODULE->local_symtab ); /* search in local tab */
...
*----------------------------------------------------------------------------*/
static SymbolHash* _select_module_symbols(Module* module,
        bool(*cond)(Symbol* sym))
{
    ModuleListElem* iter;
...
void copy_static_syms( void )
{
    ...
    for ( iter = SymbolHash_first( static_symtab ); iter;
          iter = SymbolHash_next( iter ) ) {
        sym = ( Symbol* )iter->value;
...
*----------------------------------------------------------------------------*/
static Symbol* define_local_symbol(const char* name, long value,
                                   sym_type_t type)
{
    Symbol* sym;
    ...
    if ( sym == NULL ) {                /* Symbol not declared as local */
        ...
        sym->is_defined = true;
        sym->module  =
            CURRENTMODULE;                       /* owner of symbol is always creator */
...
Symbol* define_symbol(const char* name, long value, sym_type_t type)
{
    ...
    if (sym ==
        NULL)                        /* Symbol not declared as global/extern */
...
void declare_global_symbol(const char* name)
{
    ...
    if (sym == NULL) {
        ...
        if (sym == NULL) {
            ...
            sym = Symbol_create(name, 0, TYPE_UNKNOWN, SCOPE_GLOBAL, CURRENTMODULE,
                                CURRENTSECTION);
...
void declare_public_symbol(const char* name)
{
    ...
    if (sym == NULL) {
        ...
        if (sym == NULL) {
            /* not local, not global -> declare symbol as global */
            sym = Symbol_create(name, 0, TYPE_UNKNOWN, SCOPE_PUBLIC, CURRENTMODULE,
                                CURRENTSECTION);
            SymbolHash_set(&global_symtab, name, sym);
        }
...
*----------------------------------------------------------------------------*/
static void _write_symbol_file(const char* filename, Module* module,
                               bool(*cond)(Symbol* sym),
...
*----------------------------------------------------------------------------*/
static bool cond_all_symbols(Symbol* sym)
{
    return true;
}
