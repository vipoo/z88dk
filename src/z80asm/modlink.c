...
int LinkLibModule(struct libfile* library, long curmodule, const char* modname,
                  StrHash* extern_syms);
                        ...
                        str_t* section_name, str_t* name, str_t* def_filename)
{
    ...
    while (true) {
        ...
        switch ( scope ) {
        case 'L':
            sym = define_local_sym(str_data(name), value, type);
            break;

        case 'G':
            sym = define_global_sym(str_data(name), value, type);
            break;
                                    ...
                                    str_t* section_name, str_t* target_name)
{
    ...
    while (1) {
        ...
        xfread_bcount_str(target_name,
                          file);   /* get expression EQU target, if not empty */
        ...
        if ( ( expr = expr_parse() ) != NULL ) {
            ...
            switch ( type ) {
            case 'U':
                expr->range = RANGE_BYTE_UNSIGNED;
                break;

            case 'S':
                expr->range = RANGE_BYTE_SIGNED;
                break;
            ...
            case 'B':
                expr->range = RANGE_WORD_BE;
                break;

            case 'L':
                expr->range = RANGE_DWORD;
                break;

            case '=':
                expr->range = RANGE_WORD;
                xassert( str_len(target_name) > 0 );
                expr->target_name = spool_add( str_data(
                                                   target_name) );   /* define expression as EQU */
                break;
...
static void read_module_exprs( ExprList* exprs )
{
    ...
    do {
        ...
        if (!file)
            ...
            fseek(file, fptr_base + 8,
                  SEEK_SET);           /* point at module name  pointer   */
            /*fptr_modname*/  xfread_dword(
                file);           /* get file pointer to module name */
            fptr_exprdecl = xfread_dword(
                                file);         /* get file pointer to expression declarations */
            /*fptr_namedecl*/ xfread_dword(
                file);           /* get file pointer to name declarations */
            /*fptr_libnmdecl*/xfread_dword(
                file);           /* get file pointer to library name declarations */
            ...
            if (!module_relative_addr || /* link phase */
                (Expr_is_local_in_section(expr, CURRENTMODULE, CURRENTSECTION)
                 ...
                 Expr_without_addresses(
                     expr))      /* expression with more than one address - needs to be computed at link time */
               ) {
...
void compute_equ_exprs( ExprList* exprs, bool show_error,
                        bool module_relative_addr )
{
    int  compute_result;
...
static void patch_exprs( ExprList* exprs )
{
    ...
    while ( iter != NULL ) {
        expr = iter->obj;
        xassert( expr->target_name ==
                 NULL );       /* EQU expressions are already computed */
...
static void relocate_symbols_symtab( SymbolHash* symtab )
{
    ...
    for ( iter = SymbolHash_first( symtab ); iter;
          iter = SymbolHash_next( iter ) ) {
...
static bool linked_module(struct libfile* lib, FILE* file, long obj_fpos,
                          StrHash* extern_syms)
{
    ...
    for (lib = libraryhdr->firstlib; !linked && lib != NULL; lib = lib->nextlib) {
        ...
        if (!file)
            ...
            for (obj_fpos = 8; !linked && obj_fpos != -1; obj_fpos = obj_next_fpos) {
                ...
                obj_next_fpos = xfread_dword(
                                    file);         /* get file pointer to next module in library */
...
void link_modules( void )
{
    ...
    for (module = first_obj_module; module != NULL;
         module = get_next_module(&iter)) {
        set_cur_module(module);
...
static int LinkModule_1(const char* filename, long fptr_base,
                        str_t* section_name, StrHash* extern_syms)
{
    ...
    if (!file)
        ...
        if (fptr_modcode != -1) {
            fseek(file, fptr_base + fptr_modcode,
                  SEEK_SET);  /* set file pointer to module code */
        ...
        if (fptr_namedecl != -1) {
            fseek(file, fptr_base + fptr_namedecl,
                  SEEK_SET);  /* set file pointer to point at name
                                                               * declarations */
        ...
        if (fptr_libnmdecl != -1) {
            ...
            for (p = fptr_libnmdecl; p < fptr_modname;) {
                fseek(file, fptr_base + p,
                      SEEK_SET);           /* set file pointer to point at external name declaration */
                xfread_bcount_str(name,
                                  file);                  /* read library reference name */
                p += 1 + str_len(name);                         /* point to next name */
                name_p = spool_add(str_data(name));
                StrHash_set(&extern_syms, name_p,
                            (void*)name_p);       /* remember all extern references */
            }
    ...
    return LinkTracedModule( filename,
                             fptr_base );       /* Remember module for pass2 */
}
                  ...
                  StrHash* extern_syms)
{
    ...
    flag = LinkModule(library->libfilename, curmodule,
                      extern_syms);       /* link module & read names */
...
void CreateBinFile( void )
{
    ...
    if ( opts.bin_file )        /* use predined output filename from command line */
        ...
        filename = get_bin_filename( get_first_module(
                                         NULL)->filename );        /* add '.bin' extension */

    /* binary output to filename.bin */
    binaryfile = xfopen( filename, "wb" );
    inital_binaryfile = binaryfile;

    relocfile = opts.relocatable ? NULL : opts.reloc_info ? xfopen(
                    get_reloc_filename(filename), "wb") : NULL;
    ...
    if (binaryfile) {
        ...
        fwrite_codearea(filename, &binaryfile,
                        &relocfile);     /* write code as one big chunk */
...
int LinkTracedModule(const char* filename, long baseptr )
{
    ...
    newm->moduleinfo =
        CURRENTMODULE;   /* pointer to current (active) module structure   */
    ...
    if ( linkhdr->firstlink == NULL ) {
        ...
        linkhdr->lastlink->nextlink =
            newm;     /* current/last linked module points now at new current */
        linkhdr->lastlink =
            newm;               /* pointer to current linked module updated */
    }
...
static void rename_module_local_symbols(Module* module)
{
    ...
    for (sym_it = SymbolHash_first(module->local_symtab); sym_it != NULL;
         sym_it = SymbolHash_next(sym_it)) {
    ...
    for (expr_it = ExprList_first(module->exprs); expr_it != NULL;
         expr_it = ExprList_next(expr_it)) {
        expr = expr_it->obj;
...
static void merge_codearea()
{
    ...
    for (section = get_first_section(&iter); section != NULL;
         section = get_next_section(&iter)) {
        intArray_set_size(section->module_start,
                          1);        /* delete all module boundaries */
    }
}
