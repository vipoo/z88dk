...
static void ReadNames_1(const char* filename, FILE* file,
    ...
    while (true) {
        scope = xfread_uint8(file);
        if ( scope == 0 )
            break;								/* terminator */

        symbol_char	= xfread_uint8(file);		/* type of name   */

        xfread_count8str_utstring(section_name, file);	/* read section name */

        value		= xfread_int32(file);		/* read symbol (long) integer */
        xfread_count8str_utstring(name, file);			/* read symbol name */

        // read symbol definition location
        xfread_count8str_utstring(def_filename, file);
        line_num = xfread_int32(file);

        new_section( utstring_body(section_name) );		/* define CURRENTSECTION */
        ...
        switch ( scope ) {
        case 'L': sym = define_local_sym(utstring_body(name), value, type); break;
        case 'G': sym = define_global_sym(utstring_body(name), value, type); break;
        default:
            error_not_obj_file( filename );
        }
        ...
        if (sym)
            sym->location = (Location) { str_pool_add(utstring_body(def_filename)), line_num };
    }
}

void ReadNames(const char* filename, FILE* file) {
    UT_string* section_name = utstring_alloc();
    UT_string* name = utstring_alloc();
    UT_string* def_filename = utstring_alloc();

    ReadNames_1(filename, file,
                section_name, name, def_filename);

    utstring_free(section_name);
    utstring_free(name);
    utstring_free(def_filename);
}
...
static void read_cur_module_exprs_1(ExprList* exprs, FILE* file, char* filename,
    ...
    while (1) {
        type = xfread_uint8(file);
        if ( type == 0 )
            break;			/* end marker */

        /* source file and line number */
        xfread_count16str_utstring(source_filename, file);
        if ( utstring_len(source_filename) == 0 )
            utstring_set( source_filename, utstring_body(last_filename) );
        else
            utstring_set( last_filename, utstring_body(source_filename) );

        line_num = xfread_int32(file);

        /* patch location */
        xfread_count8str_utstring(section_name, file);

        asmpc		= xfread_uint16(file);
        code_pos	= xfread_uint16(file);

        xfread_count8str_utstring(target_name, file);	/* get expression EQU target, if not empty */
        xfread_count16str_utstring(expr_text, file);		/* get expression */

        /* read expression followed by newline */
        utstring_printf(expr_text, "\n");
        SetTemporaryLine( utstring_body(expr_text) );			/* read expression */
        ...
        set_asmpc_env( CURRENTMODULE, utstring_body(section_name),
                       utstring_body(source_filename), line_num,
                       asmpc,
                       false );
        if ( ( expr = expr_parse() ) != NULL ) {
            ...
            case '=': expr->range = RANGE_WORD;
                assert( utstring_len(target_name) > 0 );
                expr->target_name = str_pool_add( utstring_body(target_name) );	/* define expression as EQU */
                break;
            default:
                error_not_obj_file( filename );
            }
            ...
            expr->location = (Location) { str_pool_add(utstring_body(source_filename)), line_num };
            expr->listpos	= -1;
...
static void read_cur_module_exprs(ExprList* exprs, FILE* file, char* filename) {
    UT_string* expr_text = utstring_alloc();
    UT_string* last_filename = utstring_alloc();
    UT_string* source_filename = utstring_alloc();
    UT_string* section_name = utstring_alloc();
    UT_string* target_name = utstring_alloc();

    read_cur_module_exprs_1(exprs, file, filename,
                            expr_text, last_filename, source_filename, section_name, target_name);

    utstring_free(expr_text);
    utstring_free(last_filename);
    utstring_free(source_filename);
    utstring_free(section_name);
    utstring_free(target_name);
...
static void read_module_exprs( ExprList* exprs ) {
    ...
    do {
        ...
        if (!file)
            ...
            /*fptr_modname*/  xfread_int32(file);			/* get file pointer to module name */
            fptr_exprdecl = xfread_int32(file);			/* get file pointer to expression declarations */
            /*fptr_namedecl*/ xfread_int32(file);			/* get file pointer to name declarations */
            /*fptr_libnmdecl*/xfread_int32(file);			/* get file pointer to library name declarations */
...
static bool linked_module(struct libfile* lib, FILE* file, long obj_fpos, StrHash* extern_syms) {
    ...
    UT_string* module_name = utstring_alloc();
    UT_string* def_filename = utstring_alloc();
    UT_string* symbol_name = utstring_alloc();
    UT_string* section_name = utstring_alloc();

    /* read module name */
    fseek(file, obj_fpos + 8, SEEK_SET);
    modname_fpos = xfread_int32(file);
    fseek(file, obj_fpos + modname_fpos, SEEK_SET);
    xfread_count8str_utstring(module_name, file);

    /* names section */
    fseek(file, obj_fpos + 8 + 4 + 4, SEEK_SET);
    names_fpos = xfread_int32(file);
    ...
    if (names_fpos != -1) {
        ...
        for (found_symbol = false; !found_symbol;) {
            ...
            xfread_uint8(file);			/* type */
            xfread_count8str_utstring(section_name, file);
            xfread_int32(file);			/* value */
            xfread_count8str_utstring(symbol_name, file);
            xfread_count8str_utstring(def_filename, file);
            xfread_int32(file);			/* line_num */

            if (scope == 'G' && StrHash_exists(extern_syms, utstring_body(symbol_name)))
                found_symbol = true;
        }

        /* link module if found one needed symbol */
        if (found_symbol) {
            LinkLibModule(lib, obj_fpos, utstring_body(module_name), extern_syms);
            linked = true;
        }
    }

    utstring_free(module_name);
    utstring_free(def_filename);
    utstring_free(symbol_name);
    utstring_free(section_name);
    ...
    for (lib = libraryhdr->firstlib; !linked && lib != NULL; lib = lib->nextlib) {
        ...
        if (!file)
            ...
            for (obj_fpos = 8; !linked && obj_fpos != -1; obj_fpos = obj_next_fpos) {
                ...
                obj_next_fpos = xfread_int32(file);			/* get file pointer to next module in library */
                module_size = xfread_int32(file);			/* get size of current module */
...
static int LinkModule_1(const char* filename, long fptr_base, UT_string* section_name, StrHash* extern_syms) {
    ...
    if (!file)
        ...
        fptr_modname = xfread_int32(file);	/* get file pointer to module name */
        xfread_int32(file);	/* get file pointer to expression declarations */
        fptr_namedecl = xfread_int32(file);	/* get file pointer to name declarations */
        fptr_libnmdecl = xfread_int32(file);	/* get file pointer to library name declarations */
        fptr_modcode = xfread_int32(file);	/* get file pointer to module code */

        if (fptr_modcode != -1) {
            fseek(file, fptr_base + fptr_modcode, SEEK_SET);  /* set file pointer to module code */

            while (true) {	/* read sections until end marker */
                code_size = xfread_int32(file);
                if (code_size < 0)
                    break;
                ...
                xfread_count8str_utstring(section_name, file);
                section = new_section(utstring_body(section_name));
                read_origin(file, section);
                section->align = xfread_int32(file);

                /* if creating relocatable code, ignore origin */
                if (opts.relocatable && section->origin >= 0) {
                    warn_org_ignored(filename, utstring_body(section_name));
                    section->origin = -1;
        ...
        if (fptr_libnmdecl != -1) {
            ...
            for (p = fptr_libnmdecl; p < fptr_modname;) {
                ...
                xfread_count8str_utstring(name, file);					/* read library reference name */
                p += 1 + utstring_len(name);							/* point to next name */
                name_p = str_pool_add(utstring_body(name));
...
void CreateBinFile( void ) {
    OpenFile bin_file = { NULL, NULL };
    OpenFile reloc_file = { NULL, NULL };
    OpenFile initial_bin_file = { NULL, NULL };
