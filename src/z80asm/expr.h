...
struct Module*
    module;         /* module where expression is patched (weak ref) */
struct Section*
    section;        /* section where expression is patched (weak ref) */
...
long
listpos;           /* position in listing file to patch (in pass 2), -1 if not listing */
END_CLASS;
...
extern bool Expr_is_local_in_section(Expr* self, struct Module* module,
