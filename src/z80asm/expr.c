...
static void ExprOp_init_operator(   ExprOp* self, tokid_t tok,
                                    op_type_t op_type );
...
void Expr_init (Expr* self)
{
    ...
    self->asmpc = get_phased_PC() >= 0 ? get_phased_PC() :
                  get_PC();    /* BUG_0048 */
...
static bool Expr_parse_factor( Expr* self )
{
    ...
    case TK_NAME:
        ...
        Str_append_n(self->text, sym.tstart,
                     sym.tlen);     /* add identifier to infix expr */
...
Expr* expr_parse( void )
{
    ...
    if ( sym.tok ==
         TK_CONST_EXPR ) {   /* leading '#' : ignore relocatable address expression */
        Str_append_n(self->text, sym.tstart, sym.tlen);
...
bool Expr_is_local_in_section(Expr* self, struct Module* module,
