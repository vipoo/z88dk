...
typedef struct OpenStruct {
    ...
    Location location;			// file and line where token found
...
struct Expr* parse_expr(const char* expr_text) {
    ...
    UT_string* expr_text_nl = utstring_alloc();
    utstring_set(expr_text_nl, expr_text);
    utstring_printf(expr_text_nl, "\n");

    save_scan_state();
    in_push();
    {
        SetTemporaryLine(utstring_body(expr_text_nl));
        num_errors = error_count();
        EOL = false;
        scan_expect_operands();
        GetSym();
        expr = expr_parse();		/* may output error */
        if (sym.tok != TK_NEWLINE && num_errors == error_count())
            error_syntax();
    }
    in_pop();
    restore_scan_state();

    utstring_free(expr_text_nl);
...
static char* token_strings_add(ParseCtx* ctx, char* str) {
    ...
    utarray_str_push_back(ctx->token_strings, str);
...
static void start_struct(ParseCtx* ctx, tokid_t open_tok, bool condition) {
    ...
    os.location = in_location(LocationAsm);
    os.active = condition;
    if (os.active)
...
static void parseline(ParseCtx* ctx) {
    ...
    start_num_errors = error_count();

    scan_expect_opcode();
    GetSym();

    if (error_count() != start_num_errors)		/* detect errors in GetSym() */
        Skipline();
    else if (!parse_statement(ctx)) {
        if (error_count() == start_num_errors)	/* no error output yet */
            error_syntax();
...
bool parse_file(const char* filename) {
    ...
    in_push();
    {
        if (in_open(filename)) {
            ...
            if (opts.verbose)
                printf("Reading '%s' = '%s'\n",
                       path_canon(filename),
                       path_canon(in_location(LocationAsm).filename));
    ...
    return num_errors == error_count() ? true : false;
}
