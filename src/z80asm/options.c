...
*----------------------------------------------------------------------------*/
DEFINE_init_module() {
    opts.inc_path = utarray_str_alloc();
    opts.lib_path = utarray_str_alloc();
    opts.files = utarray_str_alloc();
}
...
static void process_env_options() {
    ...
    UT_array* args = utarray_str_alloc();
    utarray_str_push_back(args, "z80asm");

    char* opts_copy = xstrdup(opts);		// strtok needs writeable string
    char* tok = strtok(opts_copy, " ");
    while (tok) {
        utarray_str_push_back(args, tok);
        tok = strtok(NULL, " ");
    }

    // parse args
    int argc = utarray_len(args);
    int arg;
    process_options(&arg, argc, (char**)utarray_eltptr(args, 0));
            ...
            if ((opt_arg_ptr = check_option(argv[II], opts_lu[j].long_opt)) != NULL ||
                ...
                case OptStringList:
                    if (*opt_arg_ptr) {
                        ...
                        opt_arg_ptr = expand_environment_variables(opt_arg_ptr);
                        utarray_str_push_back(*p_path, opt_arg_ptr);
                    }
                    else
                        error_illegal_option(argv[II]);
...
void expand_source_glob(const char* pattern) {
    if (strpbrk(pattern, "*?") != NULL) {		// is a pattern
        UT_array* files = path_find_glob(pattern);
        ...
        while ((p = (char**)utarray_next(files, p)) != NULL)
            utarray_str_push_back(opts.files, search_source(*p));

        utarray_free(files);
    }
    else
        utarray_str_push_back(opts.files, search_source(pattern));
}
...
static void expand_list_glob_1(const char* filename) {
    in_push();
    {
        ...
        utarray_str_push_back(opts.inc_path, path_dir(filename));
...
void expand_list_glob(const char* filename) {
    if (strpbrk(filename, "*?") != NULL) {		// is a pattern
        UT_array* files = path_find_glob(filename);
