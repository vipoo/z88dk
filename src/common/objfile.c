...
static void objfile_read_symbols(objfile_t* obj, FILE* fp, long fpos_start,
                                 long fpos_end)
{
...
static void objfile_read_externs(objfile_t* obj, FILE* fp, long fpos_start,
                                 long fpos_end)
{
    str_t* name = str_new();
...
static void objfile_read_exprs(objfile_t* obj, FILE* fp, long fpos_start,
                               long fpos_end)
{
...
// object file write
//-----------------------------------------------------------------------------
static long objfile_write_exprs1(objfile_t* obj, FILE* fp, str_t* last_filename,
                                 str_t* empty)
{
...
// read file
//-----------------------------------------------------------------------------
static void file_read_object(file_t* file, FILE* fp, str_t* signature,
                             int version)
{
...
static void file_read_library(file_t* file, FILE* fp, str_t* signature,
                              int version)
{
    ...
    do {
        ...
        file_type_e type = read_signature(fp, str_data(file->filename), obj_signature,
                                          &obj_version);
...
void file_read(file_t* file, const char* filename)
{
    ...
    file->type = read_signature(fp, str_data(file->filename), signature,
                                &file->version);
    ...
    switch (file->type) {
    case is_object:
        file_read_object(file, fp, signature, file->version);
        break;

    case is_library:
        file_read_library(file, fp, signature, file->version);
        break;
...
void file_write(file_t* file, const char* filename)
{
    ...
    switch (file->type) {
    case is_object:
        file_write_object(file, fp);
        break;

    case is_library:
        file_write_library(file, fp);
        break;
...
void file_rename_sections(file_t* file, const char* old_regexp,
                          const char* new_name)
{
    if (opt_obj_verbose)
...
static void obj_rename_symbol(objfile_t* obj, const char* old_name,
                              const char* new_name)
{
...
void file_add_symbol_prefix(file_t* file, const char* regexp,
                            const char* prefix)
{
    if (opt_obj_verbose)
    ...
    DL_FOREACH(file->objs, obj) {
        ...
        DL_FOREACH(obj->sections, section) {
            ...
            DL_FOREACH(section->symbols, symbol) {
                if (symbol->scope == 'G') {
                    if ((reti = regexec(&regex, str_data(symbol->name), 0, NULL,
                                        0)) == REG_OKAY) { // match
...
void file_rename_symbol(file_t* file, const char* old_name,
                        const char* new_name)
{
    if (opt_obj_verbose)
