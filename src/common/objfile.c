...
static file_type_e read_signature(FILE* fp, const char* filename,
    ...
    *version = -1;
    ...
    if (fread(file_signature, sizeof(char), SIGNATURE_SIZE, fp) != SIGNATURE_SIZE)
        die("error: signature not found in '%s'", filename);
    ...
    if (strncmp(file_signature, SIGNATURE_OBJ, 6) == 0)
        ...
        die("error: '%s' not object nor library", filename);
...
static void write_signature(FILE* fp, file_type_e type) {
    UT_string* signature = utstring_alloc();

    utstring_set_printf(signature,
                        "%s" SIGNATURE_VERS,
...
static section_t* read_section(objfile_t* obj, FILE* fp) {
    ...
    xfread_count8str_utstring(name, fp);
...
symbol_t* symbol_new() {
    ...
    self->name = utstring_alloc();
    self->scope = self->type = '\0';
    self->value = 0;
    self->section = NULL;
    self->filename = utstring_alloc();
    self->line_num = 0;
...
expr_t* expr_new() {
    ...
    self->text = utstring_alloc();
    self->type = '\0';
    self->asmpc = self->patch_ptr = 0;
    self->section = NULL;
    self->target_name = utstring_alloc();
...
objfile_t* objfile_new() {
    ...
    self->filename = utstring_alloc();
    self->signature = utstring_alloc();
    self->modname = utstring_alloc();

    self->version = self->global_org = -1;
    self->externs = utarray_str_alloc();
...
static void objfile_read_sections(objfile_t* obj, FILE* fp, long fpos_start) {
    ...
    if (obj->version >= 5) {
        while (true) {
            ...
            utstring_set(section->name, utstring_body(name));
            utstring_free(name);
            if (obj->version >= 8)
                section->org = xfread_int32(fp);
            else
                section->org = -1;

            if (obj->version >= 10)
                section->align = xfread_int32(fp);
            else
                section->align = -1;
        ...
        int code_size = xfread_uint16(fp);
        if (code_size == 0)
            code_size = 0x10000;
...
static void objfile_read_symbols(objfile_t* obj, FILE* fp, long fpos_start, long fpos_end) {
    ...
    while (ftell(fp) < fpos_end) {
        ...
        symbol->value = xfread_int32(fp);
        xfread_count8str_utstring(symbol->name, fp);

        if (obj->version >= 9) {			// add definition location
            xfread_count8str_utstring(symbol->filename, fp);
            symbol->line_num = xfread_int32(fp);
        }
...
static void objfile_read_externs(objfile_t* obj, FILE* fp, long fpos_start, long fpos_end) {
    ...
    while (ftell(fp) < fpos_end) {
        xfread_count8str_utstring(name, fp);
        utarray_str_push_back(obj->externs, utstring_body(name));
...
static void objfile_read_exprs(objfile_t* obj, FILE* fp, long fpos_start, long fpos_end) {
    ...
    while (ftell(fp) < fpos_end) {
        ...
        if (obj->version >= 3) {
            expr->asmpc = xfread_uint16(fp);

            if (show_expr)
                printf(" $%04X", expr->asmpc);
        }

        expr->patch_ptr = xfread_uint16(fp);
        if (show_expr)
            printf(" $%04X: ", expr->patch_ptr);

        if (obj->version >= 6) {
            xfread_count8str_utstring(expr->target_name, fp);
        ...
        if (obj->version >= 4)
            xfread_count16str_utstring(expr->text, fp);
        else {
            xfread_count8str_utstring(expr->text, fp);
            char end_marker = xfread_uint8(fp);
            if (end_marker != '\0')
...
void objfile_read(objfile_t* obj, FILE* fp) {
    ...
    if (obj->version >= 8)
        ...
        obj->global_org = xfread_int32(fp);
    else
        obj->global_org = xfread_uint16(fp);

    // file pointers
    long fpos_modname = xfread_int32(fp);
    long fpos_exprs = xfread_int32(fp);
    long fpos_symbols = xfread_int32(fp);
    long fpos_externs = xfread_int32(fp);
    long fpos_sections = xfread_int32(fp);

    // module name
    xfseek(fp, fpos0 + fpos_modname, SEEK_SET);
    xfread_count8str_utstring(obj->modname, fp);
    if (opt_obj_list)
    ...
    DL_FOREACH(obj->sections, section) {
        ...
        DL_FOREACH(section->exprs, expr) {
            ...
            if (strcmp(utstring_body(expr->filename), utstring_body(last_filename)) != 0) {
                xfwrite_count16str_utstring(expr->filename, fp);
                utstring_set(last_filename, utstring_body(expr->filename));
            }
            else
                xfwrite_count16str_utstring(empty, fp);

            xfwrite_int32(expr->line_num, fp);					// source line number
            xfwrite_count8str_utstring(expr->section->name, fp);// section name

            xfwrite_uint16(expr->asmpc, fp);						// ASMPC
            xfwrite_uint16(expr->patch_ptr, fp);					// patchptr
            xfwrite_count8str_utstring(expr->target_name, fp);	// target symbol for expression
            xfwrite_count16str_utstring(expr->text, fp);		// expression
        }
    }

    if (has_exprs) {
        xfwrite_uint8(0, fp);			// store end-terminator
        return fpos0;
    }
    else
        return -1;
...
static long objfile_write_exprs(objfile_t* obj, FILE* fp) {
    UT_string* last_filename = utstring_alloc();
    UT_string* empty = utstring_alloc();
...
static long objfile_write_symbols(objfile_t* obj, FILE* fp) {
    ...
    DL_FOREACH(obj->sections, section) {
        ...
        DL_FOREACH(section->symbols, symbol) {
            ...
            xfwrite_uint8(symbol->scope, fp);		// scope
            xfwrite_uint8(symbol->type, fp);			// type
            xfwrite_count8str_utstring(symbol->section->name, fp);// section
            xfwrite_int32(symbol->value, fp);		// value
            xfwrite_count8str_utstring(symbol->name, fp);	// name
            xfwrite_count8str_utstring(symbol->filename, fp);// filename
            xfwrite_int32(symbol->line_num, fp);		// definition line
        }
    }
...
static long objfile_write_externs1(objfile_t* obj, FILE* fp, UT_string* name) {
    ...
    while ((pname = (char**)utarray_next(obj->externs, pname)) != NULL) {
        utstring_set_printf(name, "%s", *pname);
        xfwrite_count8str_utstring(name, fp);
    }
...
static long objfile_write_modname(objfile_t* obj, FILE* fp) {
    long fpos0 = ftell(fp);
    xfwrite_count8str_utstring(obj->modname, fp);
    return fpos0;
}
...
static long objfile_write_sections(objfile_t* obj, FILE* fp) {
    ...
    section_t* section;
    DL_FOREACH(obj->sections, section) {
        xfwrite_int32(utarray_len(section->data), fp);
        xfwrite_count8str_utstring(section->name, fp);
        xfwrite_int32(section->org, fp);
        xfwrite_int32(section->align, fp);
        xfwrite_bytes(utarray_front(section->data), utarray_len(section->data), fp);
    }

    xfwrite_int32(-1, fp);					// end marker
...
#define Write_ptr(x, fp)	xfwrite_int32((x) == -1 ? -1 : (x) - fpos0, fp)
...
file_t* file_new() {
    ...
    file->filename = utstring_alloc();
    file->signature = utstring_alloc();
    file->type = is_none;
    file->version = -1;
    file->objs = NULL;
...
static void file_read_object(file_t* file, FILE* fp, UT_string* signature, int version) {
    ...
    utstring_set(obj->filename, utstring_body(file->filename));
    utstring_set(obj->signature, utstring_body(signature));
    obj->version = version;
...
static void file_read_library(file_t* file, FILE* fp, UT_string* signature, int version) {
    utstring_set(file->signature, utstring_body(signature));
    file->version = version;

    UT_string* obj_signature = utstring_alloc();
    ...
    do {
        ...
        next = xfread_int32(fp);
        length = xfread_int32(fp);

        file_type_e type = read_signature(fp, utstring_body(file->filename), obj_signature, &obj_version);
        if (type != is_object)
            die("File %s: contains non-object file", utstring_body(file->filename));
...
void file_read(file_t* file, const char* filename) {
    ...
    // save file name
    utstring_set(file->filename, filename);
...
static void file_write_library(file_t* file, FILE* fp) {
    ...
    for (objfile_t* obj = file->objs; obj; obj = obj->next) {
        long header_ptr = ftell(fp);
        xfwrite_int32(-1, fp);			// place holder for next
        xfwrite_int32(-1, fp);			// place holder for size
        ...
        xfseek(fp, header_ptr, SEEK_SET);
        if (obj->next)
            xfwrite_int32(obj_end, fp);		// next
        else
            xfwrite_int32(-1, fp);			// last
        xfwrite_int32(obj_end - obj_start, fp);
...
#define merged_section (*p_merged_section)
    ...
    utstring_set(section->name, new_name);
...
void file_rename_sections(file_t* file, const char* old_regexp, const char* new_name) {
    ...
    if (reti)
        die("error: could not compile regex '%s'", old_regexp);
    ...
    DL_FOREACH(file->objs, obj) {
        ...
        DL_FOREACH_SAFE(obj->sections, section, tmp) {
            if (strcmp(utstring_body(section->name), new_name) == 0 ||
                ...
                die("error: regex match failed: %s", msgbuf);
            }
        }
    }
...
static void obj_rename_symbol(objfile_t* obj, const char* old_name, const char* new_name) {
    UT_string* new_text = utstring_alloc();
    ...
    DL_FOREACH(obj->sections, section) {
        ...
        DL_FOREACH(section->exprs, expr) {
            if (strcmp(utstring_body(expr->target_name), old_name) == 0)
                utstring_set(expr->target_name, new_name);
            ...
            while (n < utstring_len(expr->text) &&
                ...
                if ((p == utstring_body(expr->text) || !isalnum(p[-1])) &&
                    ...
                    utstring_set_printf(new_text, "%.*s%s%s",
                                        p - utstring_body(expr->text), utstring_body(expr->text),
                                        new_name,
                                        p + strlen(old_name));
                    utstring_set(expr->text, utstring_body(new_text));
...
void file_add_symbol_prefix(file_t* file, const char* regexp, const char* prefix) {
    ...
    if (reti)
        die("error: could not compile regex '%s'", regexp);

    // search file for symbols that match
    UT_string* new_name = utstring_alloc();
            ...
            DL_FOREACH(section->symbols, symbol) {
                if (symbol->scope == 'G') {
                    if ((reti = regexec(&regex, utstring_body(symbol->name), 0, NULL, 0)) == REG_OKAY) {	// match
                        utstring_set_printf(new_name, "%s%s", prefix, utstring_body(symbol->name));
                        ...
                        utstring_set(symbol->name, utstring_body(new_name));
                    }
                    else if (reti == REG_NOMATCH) {		// no match
                        if (opt_obj_verbose)
            ...
            DL_FOREACH(section->symbols, symbol) {
                if (symbol->scope == 'G') {
                    if (strcmp(utstring_body(symbol->name), old_name) == 0) {	// match
                        ...
                        obj_rename_symbol(obj, old_name, new_name);
                        utstring_set(symbol->name, new_name);
                    }
                    else {		// no match
                        if (opt_obj_verbose)
                            printf("  skip symbol %s\n", utstring_body(symbol->name));
